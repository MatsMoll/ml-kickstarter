...
def signature_for_model(model_contract_name: str, store: FeatureStore) -> ModelSignature:
    ...
    def mlflow_spec(feature: Feature, location: FeatureLocation):
        ...
        if dtype.name == "float":
            ...
            return TensorSpec(type=np.dtype(np.float32), shape=(-1, dtype.embedding_size()), name=ref_id)
        return dtype.name
    ...
    for request in input_reqs:
        for feature in request.returned_features:
            ...
            all_features.append(
                mlflow_spec(feature, request.location)
            )
    ...
    def store_model(self, model: Model, model_name: str, store: FeatureStore) -> str:
        ...
        if isinstance(model, BaseEstimator):
            info = mlflow.sklearn.log_model(
                model,
                model_name,
                signature=signature_for_model(model_name, store)
            )
        else:
